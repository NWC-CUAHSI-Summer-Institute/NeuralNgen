...
def build_data_dict():
    ...
    # 2) Load categories information (clusters)
    txt = None
...
from BaseDataset_Fatemh_Arman import build_data_dict
from plot import plot_cluster_histogram, plot_dataframe_info

def main():
    master_dict = build_data_dict()

    # Example: Visualize cluster histogram
    raw = master_dict["clusters"]
    pairs = []
    for line in raw:
        if ";" in line:
    ...
    plot_cluster_histogram(df_cl)

    # Example: Display info for 'attributes' data
    for fname, df in master_dict["attributes"].items():
        plot_dataframe_info(df, title=f"Attributes: {fname}")

    # You can add more calls to plotting functions for other sections
