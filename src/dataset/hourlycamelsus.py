    ...
    def _load_basin_data(self, basin: str) -> pd.DataFrame:
        ...
        for forcing in self.cfg.forcings:
            if forcing.endswith('_hourly'):
                ...
                df_forcing, _ = camelsus.load_camels_us_forcings(self.cfg.data_dir, basin, forcing)
...
# Main script to test the data loading
if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

    try:
        data_dir = Path(r"/home/jovyan/CAMELS_data_sample/")
        basin_id = "12035000"
        hourly_forcing_name = "aorc_hourly"
    except Exception:
        print("Please update the 'data_dir' path in the main script block.")
        exit()
    ...
    try:
        ...
        sample = dataset[0] 
        print("\nSample of the first data point retrieved:")
        ...
        print("\nCould not retrieve a sample. This might mean no valid data was found for the specified period.")
    except Exception as e:
        # vvv THIS BLOCK IS NOW CHANGED vvv
        print("\nAn error occurred during data retrieval. Full traceback:")
        traceback.print_exc()
