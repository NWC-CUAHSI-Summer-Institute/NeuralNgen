...
class BaseDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.period = period
        self.basin_list = [basin] if basin else utils.load_basin_file(getattr(cfg, f"{period}_basin_file"))
    ...
    def __getitem__(self, item: int) -> dict:
        ...
        sample = {
            'x_d': {k: v[start_idx:idx + 1] for k, v in self._x_d[basin].items()},
    ...
    def _create_id_to_int(self):
        self.id_to_int = {str(b): i for i, b in enumerate(np.random.permutation(self.basin_list))}
    ...
    def _load_or_create_xarray_dataset(self) -> xarray.Dataset:
        ...
        for basin in tqdm(self.basin_list, disable=self._disable_pbar):
    ...
    def collate_fn(samples: List[Dict]) -> Dict:
        ...
        for key in keys:
            if key.startswith('date'):
                ...
                batch[key] = {k: torch.stack([s[key][k] for s in samples]) for k in samples[0][key]}
            else:
